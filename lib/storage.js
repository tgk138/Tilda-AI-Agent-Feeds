...
const StorageHelper = {
  ...
  async getTextConfig() {
    ...
    // Migration from legacy keys (also when apiKey is empty but legacy exists)
    if ((!provider || !apiKey) && (data[SK.LEGACY_API_KEY] || data[SK.LEGACY_OFFICIAL_KEY])) {
      const legacyProv = data[SK.LEGACY_PROVIDER] || 'kie';
      if (!provider) {
        provider = legacyProv === 'official' ? TEXT_PROVIDERS.GOOGLE : TEXT_PROVIDERS.KIE;
      }
      if (!apiKey) {
        apiKey = legacyProv === 'official' ? data[SK.LEGACY_OFFICIAL_KEY] : data[SK.LEGACY_API_KEY];
      }
      if (apiKey) {
        await StorageHelper.set({
          [SK.TEXT_PROVIDER]: provider,
          [SK.TEXT_API_KEY]: apiKey,
          [SK.TEXT_MODEL]: model || DEFAULT_MODELS[provider] || '',
        });
      }
    }
  ...
  async getMediaConfig() {
    ...
    if ((!provider || !apiKey) && (data[SK.LEGACY_API_KEY] || data[SK.LEGACY_OFFICIAL_KEY])) {
      const legacyProv = data[SK.LEGACY_PROVIDER] || 'kie';
      if (!provider) {
        provider = legacyProv === 'official' ? MEDIA_PROVIDERS.GOOGLE_IMAGEN : MEDIA_PROVIDERS.KIE_BANANA;
      }
      if (!apiKey) {
        apiKey = legacyProv === 'official' ? data[SK.LEGACY_OFFICIAL_KEY] : data[SK.LEGACY_API_KEY];
      }
      if (apiKey) {
        await StorageHelper.set({
          ...
          [SK.MEDIA_MODEL]: model || DEFAULT_MODELS[provider] || '',
        });
      }
    }
